...
"read:agents"        })    }, tags={ "agents" })
    ...
    ResponseEntity<List<Agent>> findAgentsByStatus(@Parameter(in = ParameterIn.QUERY, description = "Status values that need to be considered for filter" ,schema=@Schema(allowableValues={"off", "ready", "busy", "afterCall", "preCall", "notReady", "break", "lunch"}
...
"read:agents"        })    }, tags={ "agents" })
    ...
    @RequestMapping(value = "/agents/{agentId}",
        produces = { "application/json"},// "application/xml" },
        method = RequestMethod.GET)
    ResponseEntity<Agent> getAgentsById(@Parameter(in = ParameterIn.PATH, description = "ID of agent to return", required=true, schema=@Schema()) @PathVariable("agentId") Long agentId);
