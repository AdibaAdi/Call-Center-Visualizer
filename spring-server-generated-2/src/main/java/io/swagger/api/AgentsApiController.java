...
public class AgentsApiController implements AgentsApi {
    ...
    public ResponseEntity<List<Agent>> findAgentsByStatus(@Parameter(in = ParameterIn.QUERY, description = "Status values that need to be considered for filter" ,schema=@Schema(allowableValues={ "ready", "busy", "afterCall", "preCall", "notReady", "break", "lunch", "out" }
, defaultValue="available")) @Valid @RequestParam(value = "status", required = false, defaultValue="available") String status) {
    ...
    public ResponseEntity<Agent> getAgentsById(@Parameter(in = ParameterIn.PATH, description = "ID of pet to return", required=true, schema=@Schema()) @PathVariable("petId") Long petId) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Agent>(objectMapper.readValue("{\n  \"firstName\" : \"John\",\n  \"lastName\" : \"James\",\n  \"password\" : \"abc12345\",\n  \"phone\" : \"12345\",\n  \"id\" : 10,\n  \"agentStatus\" : 1,\n  \"email\" : \"john@email.com\",\n  \"username\" : \"theUser\"\n}", Agent.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
